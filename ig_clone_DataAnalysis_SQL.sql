...
WHERE
    username REGEXP '^[A]';
    -- other way to do it 
SELECT COUNT(username) AS ausername FROM users WHERE username LIKE 'A%';
-- --------------------------------------------------------------------------------------------------------------

/*Ques.12 The most popular tag names by usage*/

SELECT 
    t.tag_name, COUNT(tag_name) AS used_tags
FROM
    ig_clone.tags t
        JOIN
    ig_clone.photo_tags pt ON t.id = pt.tag_id
GROUP BY t.id
ORDER BY used_tags DESC
LIMIT 10;
-- --------------------------------------------------------------------------------------------------------------
...
SELECT 
    ROUND(AVG(DATEDIFF(CURRENT_TIMESTAMP, created_at)/365), 2) as Total_Years_on_Platform
FROM
    ig_clone.users;
-- --------------------------------------------------------------------------------------------------------------
